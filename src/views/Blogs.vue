  ...
  <div class="container mx-auto p-5 py-10 sm:p-10">
    ...
    <div class="flex items-center gap-3 whitespace-nowrap mb-5">
      ...
      <button type="button" @click="openModal(null)"
        class="relative flex items-center gap-2 border-2 border-blue-600 overflow-hidden focus:outline-none leading-loose align-middle px-3 py-1.5 rounded-md cursor-pointer text-sm bg-blue-600 text-white">
      ...
      <TransitionRoot appear :show="isOpen" as="template">
        <Dialog as="div" @close="closeModal" class="relative z-10">
          ...
          <div class="fixed inset-0 overflow-y-auto font-jost">
              ...
              <TransitionChild as="template" enter="duration-300 ease-out" enter-from="opacity-0 scale-95"
                ...
                <DialogPanel
                  ...
                  <DialogTitle as="h3" class="text-lg font-medium leading-6 text-gray-900 mb-5">
                    <span v-if="form.id">Edit Blog</span>
                    <span v-else>New Blog</span>
                  </DialogTitle>
                  <form @submit.prevent="saveBlog()" class="mt-2 space-y-4">

                    <!-- show errors -->
                    <div v-if="blogStore.errors" class="text-red-500 text-sm">
                      <ul>
                        <li v-for="error in blogStore.errors" :key="error">{{ error }}</li>
                      </ul>
                    </div>
                    ...
                    <div class="max-w-full">
                      <div class="flex gap-x-6">
                        <div class="flex">
                          <input type="radio" name="radio-status" v-model="form.status" value="published"
                            ...
                            id="published">
                          <label for="published" class="text-sm text-gray-500 ms-2 dark:text-neutral-400">
                            Published
                          </label>
                        </div>
                        <div class="flex">
                          <input type="radio" name="radio-status" v-model="form.status" value="unpublished"
                            class="shrink-0 mt-0.5 border-gray-200 rounded-full text-blue-600 focus:ring-blue-500 disabled:opacity-50 disabled:pointer-events-none dark:bg-neutral-800 dark:border-neutral-700 dark:checked:bg-blue-500 dark:checked:border-blue-500 dark:focus:ring-offset-gray-800"
                            id="unpublished">
                          <label for="unpublished" class="text-sm text-gray-500 ms-2 dark:text-neutral-400">
                            Unpublished
                          </label>
                        </div>
                      </div>
                    ...
                    <!-- Author -->
                    <div class="max-w-full">
                      <label for="author" class="block text-sm font-medium mb-2 dark:text-white">Author</label>
                      <select v-model="form.author_id"
                        class="py-2.5 px-4 pe-9 block w-full border-gray-200 rounded-lg text-sm focus:border-blue-500 focus:ring-blue-500 disabled:opacity-50 disabled:pointer-events-none dark:bg-neutral-900 dark:border-neutral-700 dark:text-neutral-400 dark:placeholder-neutral-500 dark:focus:ring-neutral-600">
                    ...
                    <div class="flex gap-3 mt-7">
                      <button type="submit"
                        class="inline-flex cursor-pointer duration-200 justify-center rounded-md border border-transparent bg-blue-100 px-4 py-2 text-sm font-medium text-blue-900 hover:bg-blue-200 focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 focus-visible:ring-offset-2">
                        <span v-if="form.id">Update</span>
                        <span v-else>Create</span>
                      </button>
                      <button type="button"
                        class="inline-flex cursor-pointer duration-200 justify-center rounded-md border border-transparent bg-gray-100 px-4 py-2 text-sm font-medium text-gray-900 hover:bg-gray-200 focus:outline-none focus-visible:ring-2 focus-visible:ring-gray-500 focus-visible:ring-offset-2"
                        @click="closeModal">
                        Close
                      </button>
                    </div>
    ...
    <!-- Table -->
    <vue3-datatable ref="datatable" :rows="filterRows" :columns="cols" :loading="loading" :totalRows="total_rows"
      ...
      <template #actions="data">
        <div class="flex gap-4">
          ...
          <!-- <button type="button" class="text-red-800 cursor-pointer" @click="deleteBlog(data.value)"> -->
          <button type="button" class="text-red-800 cursor-pointer" @click="openModalDelete(data.value)">
            <svg xmlns="http://www.w3.org/2000/svg" class="size-5" viewBox="0 0 24 24">
              <path fill="currentColor"
    ...
    <!-- Delete Modal -->
    <TransitionRoot appear :show="openDelete" as="template">
      <Dialog as="div" @close="openDelete = false, resetForm()" class="relative z-10">
        <TransitionChild as="template" enter="duration-300 ease-out" enter-from="opacity-0" enter-to="opacity-100"
          leave="duration-200 ease-in" leave-from="opacity-100" leave-to="opacity-0">
          <div class="fixed inset-0 bg-black/25" />
        </TransitionChild>
          ...
          <div class="flex min-h-full items-center justify-center p-4 text-center">
            <TransitionChild as="template" enter="duration-300 ease-out" enter-from="opacity-0 scale-95"
              ...
              <DialogPanel
                ...
                <p class="text-sm text-gray-500 mb-5">Are you sure you want to delete this blog?</p>
                <div class="flex gap-3 mt-7">
                  <button type="button"
                    class="inline-flex cursor-pointer duration-200 justify-center rounded-md border border-transparent bg-red-100 px-4 py-2 text-sm font-medium text-red-900 hover:bg-red-200 focus:outline-none focus-visible:ring-2 focus-visible:ring-red-500 focus-visible:ring-offset-2"
                    @click="deleteBlog(form.id)">
                    Delete
                  </button>
                  <button type="button"
                    class="inline-flex cursor-pointer duration-200 justify-center rounded-md border border-transparent bg-gray-100 px-4 py-2 text-sm font-medium text-gray-900 hover:bg-gray-200 focus:outline-none focus-visible:ring-2 focus-visible:ring-gray-500 focus-visible:ring-offset-2"
                    @click="openDelete = false, resetForm()">
                    Cancel
                  </button>
                </div>
              </DialogPanel>
...
const setRows = () => {
  ...
  authors.value = blogStore.authors;
  filterBlogs();
};
...
function openModal(data) {
  isOpenChooser.value = false
  isOpenStatus.value = false

  if (data) {
    form.value = {
      id: data.id,
      author_id: blogStore.authors.find((author) => author.name === data.author).id,
      title: data.title,
...
const saveBlog = async () => {
  if (form.value.id) {
    await blogStore.updateBlog(form.value);
  } else {
    delete form.value.id;
    await blogStore.createBlog(form.value);
  }
  if (blogStore.errors && blogStore.errors.length > 0) {
    return;
  }
  setRows();
  closeModal();
